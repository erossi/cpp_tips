...
/*
 * Scope: define a templated normal class and use it as a static
...
template <typename T>
SimpleBufferManager<T>::SimpleBufferManager(size_t buffersize) : size_{buffersize}
{
	buffer_ = new T[size_];
}
...
template <typename T>
void SimpleBufferManager<T>::add(unsigned int position, T value)
{
	if (position < size_)
...
template <typename T>
T SimpleBufferManager<T>::get(unsigned int position)
{
	if (position < size_)
		return buffer_[position];
	else
		return 0; // 0 may not be T compatibile.
}

// The second class
class StaticBufferManager {
	private:
		static SimpleBufferManager<int> sbm_;
	public:
...
SimpleBufferManager<int> StaticBufferManager::sbm_ {5};

int main()
{
	// test the normal object.
	SimpleBufferManager<int> buffer {5};
